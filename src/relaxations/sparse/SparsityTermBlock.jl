...
struct SparsityTermBlock <: SparsityTerm
    ...
    @doc """
        SparsityTermBlock(problem::POProblem)
    ...
    See also [`poly_problem`](@ref), [`poly_optimize`](@ref), [`SparsityTermCliques`](@ref),
    ...
    the resulting object will be shared - `groupings` will always give back the result of the last iteration with the
    type that the pattern had _at the time of iteration_.

    See also [`SparsityTerm`](@ref).
    """
    function SparsityTermBlock(problem::POProblem, args...)
...
iterate!(ts::SparsityTermBlock; kwargs...) = isnothing(iterate!(ts.logic, :block; kwargs...)) ? nothing : ts
