...
for cp in (:copy!, :copyto!)
    @eval begin
        function Base.$cp(dst::PackedMatrix{R}, src::PackedMatrix{R}) where {R}
            $cp(dst.data, src.data)
            return dst
        end
        function Base.$cp(dst::PackedMatrix{R}, src::AbstractVector{R}) where {R}
            $cp(dst.data, src)
            return dst
        end
        Base.$cp(dst::AbstractVector{R}, src::PackedMatrix{R}) where {R} = $cp(dst, src.data)
    end
end
