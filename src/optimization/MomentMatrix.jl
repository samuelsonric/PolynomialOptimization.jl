...
moment_matrix(moments::MomentVector{<:Any,V,Nr,Nc}, ::Val{:symmetric}, rows, cols,
    prefix::Union{<:IntMonomialOrConj{Nr,Nc},<:IntVariable{Nr,Nc}}...) where {Nr,Nc,V} =
...
Note that `prefix` has to be a valid `IntMonomial` or `IntVariable` of appropriate type.

See also [`poly_optimize`](@ref), [`poly_optimize`](@ref).
"""
function moment_matrix(result::Result{R}; max_deg=Inf,
    prefix::Union{<:IntMonomialOrConj{Nr,Nc},<:IntVariable{Nr,Nc},Nothing}=nothing) where {Nr,Nc,R<:AbstractRelaxation{<:Problem{<:IntPolynomial{<:Any,Nr,Nc}}}}
