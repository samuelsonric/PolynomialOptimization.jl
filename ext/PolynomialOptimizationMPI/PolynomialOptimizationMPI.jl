...
import MPI, Random
import PolynomialOptimization: @verbose_info, @capture, haveMPI
import PolynomialOptimization.Newton: execute_taskfun, execute, halfpolytope, restore_status!
using PolynomialOptimization.Newton: preproc, analyze, prepare, alloc_global, alloc_local, clonetask, work,
    InitialStateIterator, moniter_state, toSigned, SimpleMonomialVector, isless_degree, monomial_cut
...
function halfpolytope(V, objective::P, comm, rank::MPIRank; verbose::Bool=false,
    ...
    if isroot(rank)
        ...
        newton_time = @elapsed allresult = let
            ...
            candidates = execute(V, size(coeffs, 1), analysis..., verbose, num, nthreads, task, secondtask, filepath, comm,
                rank)
