mutable struct StateMoment{M}
    ...
    constraints::Vector{Tuple{Vector{Int},Vector{Int},Vector{Float64},Vector{Float64},AbstractConvexSet{Float64}}}
end

get_variable!(state::StateMoment{M}, mon::M) where {M} = get!(state.variable_mappings, mon, length(state.variable_mappings) +1)
...
function moment_matrix!(state::StateMoment{M}, gröbner_basis, grouping::AbstractVector{M}, constraint::P) where {P,M}
    ...
    for exp2 in 1:lg
        for exp1 in 1:lg # COSMO is slightly more efficient for the full version
            @inbounds constr_terms = mergewith(+, (get_variable!(state, monomial(term)) => coefficient(term)
    ...
    push!(state.constraints, (finish!(rows), finish!(cols), finish!(vals), fill(0.0, i - 1), PsdCone(lg * lg)))
    return
end
...
function moment_matrix!(state::StateMoment{M}, gröbner_basis, grouping::AbstractVector{M}, constraint::AbstractMatrix{P}) where {P,M}
    ...
    push!(state.constraints, (finish!(rows), finish!(cols), finish!(vals), fill(0.0, i - 1), PsdCone(dim * dim)))
    return
end

function moment_matrix_eq!(state::StateMoment{M}, gröbner_basis, grouping::AbstractVector{M}, constraint::P) where {P,M}
    lg = length(grouping)
    rows = FastVec{Int}()
    cols = FastVec{Int}()
    vals = FastVec{Float64}()
    ...
    push!(state.constraints, (finish!(rows), finish!(cols), finish!(vals), fill(0.0, i - 1), ZeroSet((lg * (lg +1)) >> 1)))
    return
end

function PolynomialOptimization.sparse_optimize(::Val{:COSMOMoment}, problem::PolyOptProblem{P,M,V},
    groupings::Vector{<:Vector{<:AbstractVector{M}}}; verbose::Bool=false, customize::Function=(state, obj) -> obj,
    parameters...) where {P,M,V}
    @assert(!problem.complex)
    model = Model()
    setup_time = @elapsed begin
        !haskey(parameters, :decompose) && (parameters = (decompose=false, parameters...))
        settings = Settings(; verbose=verbose, verbose_timing=verbose, parameters...)

        state = StateMoment(Dict{M,Int}(),
            Tuple{Vector{Int},Vector{Int},Vector{Float64},Vector{Float64},AbstractConvexSet{Float64}}[])
        ...
        if isone(problem.prefactor)
            ...
            push!(state.constraints, ([1], [get_variable!(state, constant_monomial(problem.objective))], [1.0], [-1.0],
                ZeroSet(1)))
        else
            push!(state.constraints, (fill(1, nterms(problem.prefactor)), get_variable!.((state,), monomials(problem.prefactor)),
                Float64.(coefficients(problem.prefactor)), [-1.0], ZeroSet(1)))
        end
        ...
        objective = customize(state, objective)

        num_vars = length(state.variable_mappings)
        assemble!(model, spzeros(num_vars, num_vars), sparsevec(objective..., num_vars),
            map(x -> Constraint(sparse(x[1], x[2], x[3], maximum(x[1]), num_vars), x[4], x[5]), state.constraints),
            settings=settings)
    end
