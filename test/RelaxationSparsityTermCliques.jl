...
@testset "Example 3.6" begin
    ...
    sp = RelaxationSparsityTermChordal(RelaxationCustom(
...
@testset "Example 7.2 from TSSOS with block completion" begin
    ...
    sp = RelaxationSparsityTermChordal(
...
@testset "Example 4.5 from Zhen, Fantuzzi, Papachristodoulou review" begin
    DynamicPolynomials.@polyvar x[1:3]
    sp = RelaxationSparsityTermChordal(poly_problem(1 + x[1]^4 + x[2]^4 + x[3]^4 + x[1]^2 * x[2]^2 + x[1]^2 * x[3]^2 +
...
@testset "Something with matrices" begin
    DynamicPolynomials.@polyvar x[1:3]
    sp = RelaxationSparsityTermChordal(poly_problem(1 + x[1]^4 + x[2]^4 + x[3]^4 + x[1] * x[2] * x[3] + x[2],
...
@testset "Complex-valued" begin
    ...
    sp = RelaxationSparsityTermChordal(poly_problem(z[1] + conj(z[1]), nonneg=[1 - z[1] * conj(z[1]) - z[2] * conj(z[2])]), 2)
    @test strRep(groupings(sp)) == "Groupings for the relaxation of a polynomial optimization problem
Variable cliques
================
[z₁, z₂]
