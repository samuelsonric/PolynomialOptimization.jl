...
@testset "POP 1 (Motzkin)" begin
    ...
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 5 in 2 variables
...
@testset "POP 8 (Example 3.1)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 3 in 2 variables
...
@testset "POP 10 (Example 4.4)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 7 in 2 variables
...
@testset "POP 11 (§5.2)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 3 in 8 variables
...
Size of full basis: 165"
    ...
    if optimize
        ...
        # COPT requires a license for the large problem
        # MosekMoment: unknown
        # :HypatiaMoment ∈ solvers && @test poly_optimize(:HypatiaMoment, prob, dense=true).objective ≈ -112.014 atol = 2e-3 # takes 6 minutes
    end
end
...
Size of full basis: 165"
    if optimize
        :MosekSOS ∈ solvers && @test poly_optimize(:MosekSOS, prob).objective ≈ 1.0072 atol = 2e-2
        # COPT requires a license for the large problem
        :MosekMoment ∈ solvers && @test poly_optimize(:MosekMoment, prob).objective ≈ 1.0072 atol = 2e-2
        #:HypatiaMoment ∈ solvers && @test poly_optimize(:HypatiaMoment, prob, dense=true).objective ≈ 1.0072 atol = 2e-2 # takes 3:40 minutes
    end
end
...
Size of full basis: 84"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ 0 atol = 9e-4
        end
    end
end
...
@testset "POP 15 (Example A.6)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 4 in 4 variables
...
Size of full basis: 70"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ 0 atol = 3e-5
        end
    end
end
...
1: 0 = -x₂² + x₁³
Size of full basis: 36"
    # this problem is very ill-defined, almost none of the solver will report success
    :COPTSOS ∈ solvers && @test poly_optimize(:COPTSOS, prob).objective ≈ 0.9693349 atol = 1e-6
...
Size of full basis: 91"
    ...
    # value as where MosekSOS claims unknown. COPT will work, but requires a license.)
...
Size of full basis: 171"
    if optimize
        :MosekSOS ∈ solvers && @test poly_optimize(:MosekSOS, prob).objective ≈ 0.98476 atol = 3e-3
        # COPT requires a license, Hypatia is the only one of the others that gives something close to a good result (30s)
    end
...
@testset "POP 17 (Example A.8)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 3 in 10 variables
...
@testset "POP 19 (Example 2)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 2 in 4 variables
...
Size of full basis: 15"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ 0.2708494 atol = 2e-3
        end
    end
end
...
Size of full basis: 15"
    if optimize
        # @test_broken poly_optimize(:MosekMoment, prob).objective ≈ -8.5578 atol = 1e-4
...
@testset "POP 22 (AM-GM inequality)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 3 in 3 variables
...
Size of full basis: 20"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ 3 atol = 2e-3
        end
    end
end
...
@testset "POP 23 (USSR Olimpian 1989)" begin
    ...
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 7 in 3 variables
...
Size of full basis: 120"
    if optimize
        :MosekSOS ∈ solvers && @test poly_optimize(:MosekSOS, prob).objective ≈ 2 atol = 1e-4
    end
end
...
@testset "POP 24 (IMO 1990)" begin
    ...
    prob = poly_problem(sum(x[i]*prod(sum(x[j] for j in 1:4 if j != k) for k in 1:4 if k != i) for i in 1:4) -
        ...
        noncompact=(1e-5, 0))
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 3 in 4 variables
...
Size of full basis: 35"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ 5.0625 atol = 2e-4
        end
    end
end
...
@testset "POP 25 (IMO 2000)" begin
    ...
    prob = poly_problem(-prod(x[mod(i-1, 1:3)]*x[i] - x[i] +1 for i in 1:3), 0, nonneg=x, zero=[prod(x)-1],
        noncompact=(1e-5, 1))
    map_coefficients!(x -> round(x, digits=5), prob.objective)
    @test strRep(prob) == "Real-valued polynomial optimization hierarchy of degree 5 in 3 variables
...
Size of full basis: 56"
    if optimize
        for solver in solvers
            @test poly_optimize(solver, prob).objective ≈ -0.997439 atol = 2e-4
        end
    end
end